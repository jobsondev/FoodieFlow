...
@router.get(
    "/", response_model=list[CategoriaModel], description="Busca todas as categorias"
)
...
def update_categoria(
    categoria_id: int, updated_categoria: CategoriaModel, db: Session = Depends(get_db)
):
    try:
        log.info(f"Categoria recebida para atualização: {updated_categoria}")
        categoria = categoria_service.update_categoria(
            db, categoria_id, updated_categoria
        )
        if not categoria:
            raise HTTPException(status_code=404, detail="Categoria não encontrada")
        return categoria
