...
from core.model.pedido import PedidoCompleto
...
class PedidoDatabaseAdapter(PedidoRepository):
    ...
    def create_pedido(
        ...
        pedido_data["id_status"] = (
            StatusEnum.RECEBIDO.value
        )  # Por padrão, o status é "Recebido"
        ...
        if produtos:
            produtos_orm = (
                db.query(ProdutoORM).filter(ProdutoORM.id.in_(produtos)).all()
            )
    ...
    def get_pedidos(self, db: Session, skip: int = 0, limit: int = 100):
        pedidos = (
            db.query(PedidoORM)
            .filter(PedidoORM.id_status != 4)
            .order_by(PedidoORM.id_status, PedidoORM.id)
            .offset(skip)
            .limit(limit)
            .all()
        )
    ...
    def get_pedidos_by_status(
        ...
        pedidos = (
            db.query(PedidoORM)
            .filter(PedidoORM.id_status == status)
            .offset(skip)
            .limit(limit)
            .all()
        )
