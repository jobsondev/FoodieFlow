...
class ProdutoDatabaseAdapter(ProdutoRepository):
    def _to_produto_completo(self, produto: ProdutoORM) -> ProdutoCompleto:
        return ProdutoCompleto(
            ...
            imagens=[
                Imagem(
                    caminho=imagem.caminho, id_produto=imagem.id_produto, id=imagem.id
                )
    ...
    def create_produto(
        ...
        if ingredientes:
            ingredientes_orm = (
                db.query(IngredienteORM)
                .filter(IngredienteORM.id.in_(ingredientes))
                .all()
            )
    ...
    def get_produtos_by_categoria(
        ...
        produtos = (
            ...
            .filter(ProdutoORM.id_categoria == id_categoria)
            .offset(skip)
            .limit(limit)
            .all()
        )
    ...
    def update_produto(
        ...
        updated_data = updated_produto.dict(
            exclude_unset=True, exclude={"ingredientes", "imagens"}
        )
